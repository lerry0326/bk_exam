...
def is_domain_valid(dial_url):
    ...
    try:
        ...
        resp = requests.get(dial_url, allow_redirects=False)
...
def get_csrftoken(url, token_name):
    ...
    if not is_status_ok(resp) or token is None:
        raise CsrfException(u"get {} from {} failed({}):{}".format(token_name, url, resp.status_code, resp.text))
...
def auto_login_paas(paas_domain, username, password):
    ...
    raise LoginException(u"login {} failed({}): {}.".format(paas_domain, resp.status_code, resp.text))
...
def start_deploy_app_task(paas_domain, appid, env, is_use_celery=True, is_use_celery_beat=True):
    ...
    resp = session.post(deploy_app_url, data=deploy_param
                        , headers={"X-CSRFToken": csrftoken, 'Referer': app_info_url})

    # 启动部署任务接口调用失败
    if not is_status_ok(resp):
        raise DeployException(u"start deploy {} to {} failed({}):{}".format(
            appid,
            env,
            resp.status_code,
            resp.text
    ...
    try:
        ...
        print "start deploy {} to {} success:{}, just wait for a moment to access it".format(
            appid,
            env,
            deploy_data.get('event_id')
        )
        return deploy_data.get('event_id')
    except ValueError as e:
        print resp.text
        raise DeployException(u"start deploy {} to {} exception:{}".format(
            appid,
            env,
            e.message,
        ))
...
if __name__ == '__main__':
    ...
    parser.add_argument("-d", "--domain", help="bk paas domain, such as: http://paasee-test.o.qcloud.com:80", required=True)
    ...
    print """
        ...
        PAAS_DOMAIN:    {}
        ===========================    
    """.format(APP_ID, ENV, USERNAME, PASSWORD, PAAS_DOMAIN)
