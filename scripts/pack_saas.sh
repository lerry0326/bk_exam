...
pip download --no-binary=:all: -d $APP_CODE/pkgs/ -r requirements.txt --trusted-host pypi.douban.com --no-build-isolation || exit 1

echo "create package"
# tar -zcvf "$APP_CODE-V$VERSION.tar.gz" $APP_CODE
# mac version
COPYFILE_DISABLE=1 tar -zcvf "$APP_CODE-$VERSION.tar.gz" $APP_CODE
rm -rf $APP_CODE
